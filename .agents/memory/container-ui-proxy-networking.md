...
**How to apply:** Inject the node IP into the API pod using the downward API (`status.hostIP`), inspect the running container's network addresses and published binding, and try node host-port, container IP, Docker gateway, then localhost. Service-image labs such as Jenkins need their setup files written before the service reads them; restarting the container once after setup applies init files created after the first start. Log the selected upstream target when connections fail.