...
Session start endpoints must acknowledge with a `starting` row before Docker/Jenkins provisioning completes. Holding the HTTP request open through image setup and Jenkins boot exceeds the published gateway timeout and appears to students as repeated 502 responses. Concurrent acknowledgements must share the same in-flight start promise so callers cannot race the initial database write.
