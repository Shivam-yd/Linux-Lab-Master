...
- Docker exec can briefly fail with an OCI `setns` error immediately after a container starts; retry that exact runtime error once before removing the container.
  **Why:** Jenkins service containers can be running while their process namespace is not yet ready for the first exec.
  **How to apply:** retry only `OCI runtime exec failed`/`setns` errors with a short delay; preserve normal setup exit codes and unrelated Docker errors.
