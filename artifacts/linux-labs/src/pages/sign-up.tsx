...
export default function SignUpPage() {
  ...
  async function handleRequest(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) setReqError(body.error ?? "Could not submit request.")
          ...
          {isClosed && mode === "invite_or_request" && (
            <>
              <div className="flex items-center gap-2 mb-4">
                ...
                <p className="text-xs text-amber-400 font-medium">Registration is currently restricted</p>
              </div>
              {reqDone ? (
                <div className="text-center space-y-3 py-4">
                  <p className="text-sm font-semibold text-green-400">Request submitted!</p>
                  <p className="text-sm text-muted-foreground">Your instructor will review your request.</p>
                </div>
              ) : (
                <>
                  <h1 className="text-xl font-bold text-foreground mb-1">Request an account</h1>
                  <p className="text-sm text-muted-foreground mb-6">Your instructor will review and approve your request.</p>
                  <form onSubmit={handleRequest} className="flex flex-col gap-4">
                    <div className="flex flex-col gap-1.5">
                      <Label htmlFor="req-name">Name</Label>
                      <Input id="req-name" type="text" placeholder="Your name" value={reqName} onChange={e => setReqName(e.target.value)} required />
                    </div>
                    <div className="flex flex-col gap-1.5">
                      <Label htmlFor="req-email">Email</Label>
                      <Input id="req-email" type="email" placeholder="you@example.com" value={reqEmail} onChange={e => setReqEmail(e.target.value)} required />
                    </div>
                    {reqError && <p className="text-sm text-destructive bg-destructive/10 border border-destructive/20 rounded-lg px-3 py-2">{reqError}</p>}
                    <Button type="submit" disabled={reqLoading} className="w-full mt-1">
                      {reqLoading && <Loader2 className="w-4 h-4 animate-spin mr-2" />}
                      Submit request
                    </Button>
                  </form>
                </>
