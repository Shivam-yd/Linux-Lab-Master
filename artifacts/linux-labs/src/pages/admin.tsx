                ...
                {sessions.data && (
                  <>
                    <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 mb-4">
                      {[
                        { label: "Active", value: sessions.data.summary.active, tone: "text-primary" },
                        { label: "Running", value: sessions.data.summary.running, tone: "text-emerald-400" },
                        { label: "Starting", value: sessions.data.summary.starting, tone: "text-amber-400" },
                        { label: "Failed", value: sessions.data.summary.failed, tone: "text-red-400" },
                        { label: "Stale", value: sessions.data.summary.stale, tone: "text-orange-400" },
                      ].map((metric) => (
                        <div key={metric.label} className="rounded-xl border border-border/50 bg-card/60 px-3 py-2.5">
                          <p className="text-[10px] uppercase tracking-wider text-muted-foreground">{metric.label}</p>
                          <p className={cn("text-xl font-black font-mono mt-0.5", metric.tone)}>{metric.value}</p>
                        </div>
                      ))}
                    </div>
                    {sessions.data.sessions.length === 0 ? (
                      ...
                      <>
                        <div className="flex items-center justify-between px-1 pb-1">
                          <div className="grid grid-cols-[1fr_1fr_150px_auto_auto] gap-4 px-4 text-xs font-semibold text-muted-foreground uppercase tracking-wider flex-1">
                            <span>Student</span><span>Lab</span><span>Timing</span><span className="w-20 text-center">Status</span><span className="w-16" />
                          </div>
                          <button
                        ...
                        {sessions.data.sessions.map((s: SessionRow) => {
                      ...
                      return (
                        <div key={`${s.student_id}:${s.lab_id}`} className="rounded-xl border border-border/50 bg-card/60 px-5 py-4 grid grid-cols-1 sm:grid-cols-[1fr_1fr_150px_auto_auto] gap-3 sm:gap-4 items-center">
                          ...
                          <div className="min-w-0 text-xs text-muted-foreground">
                            <p>{s.status === "starting" ? "Starting" : "Updated"} {relativeTime(s.updated_at)}</p>
                            <p className="text-[10px] text-muted-foreground/70 mt-0.5">
                              {s.started_at ? `Age ${fmtDuration(Math.max(0, s.age_seconds))}` : "No start time recorded"}
                            </p>
                            {s.error_message && (
                              <p className="text-[10px] text-red-400 truncate mt-1" title={s.error_message}>Startup failed: {s.error_message}</p>
                            )}
                          </div>
