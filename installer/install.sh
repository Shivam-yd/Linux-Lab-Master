...
# Must be run from inside the project directory
...
if [[ -f "${ENV_FILE}" ]]; then
  ...
  # that would silently abort the whole script right here. Disable
  # pipefail just for these two lines so the broken-pipe exit is ignored.
  ...
  cat > "${ENV_FILE}" <<EOF
# Auto-generated by Linux Labs installer — do not edit manually
...
[Unit]
Description=Linux Labs Learning Platform
