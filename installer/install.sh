...
# ── Step 3: Generate secrets & optional Clerk config ─────────────────────────
...
if [[ -f "${ENV_FILE}" ]]; then
  ...
  # LinuxLabMaster works out of the box without Clerk — guests get a browser
  # cookie and their progress is saved locally. If you want named user accounts
  # (email + password, Google OAuth, etc.), create a free app at clerk.com and
  # supply the keys either:
  #   a) Non-interactively (recommended for multi-machine installs):
  #        CLERK_SECRET_KEY=sk_... CLERK_PUBLISHABLE_KEY=pk_... sudo -E bash installer/install.sh
  #   b) Interactively: just press Enter at the prompts below to skip.
  if [[ -z "${CLERK_SECRET_KEY:-}" || -z "${CLERK_PUBLISHABLE_KEY:-}" ]]; then
    echo ""
    echo -e "${BOLD}Optional: Clerk authentication${RESET}"
    echo    "  Without Clerk, anyone can use labs as a guest (progress saved by cookie)."
    echo    "  With Clerk, users can create accounts and their progress is tied to their login."
    echo    "  Get free keys at https://clerk.com → create app → API Keys."
    echo    "  Tip: pre-set CLERK_SECRET_KEY and CLERK_PUBLISHABLE_KEY env vars to skip this prompt."
    echo ""
    read -rp "  Clerk Secret Key       (sk_live_... / sk_test_... or Enter to skip): " CLERK_SECRET_KEY
    read -rp "  Clerk Publishable Key  (pk_live_... / pk_test_... or Enter to skip): " CLERK_PUBLISHABLE_KEY
  else
    info "Clerk keys provided via environment — skipping interactive prompt"
  fi
  ...
  if [[ -n "${CLERK_SECRET_KEY:-}" ]]; then
    success "Secrets written to ${ENV_FILE} (Clerk authentication enabled)"
  else
    success "Secrets written to ${ENV_FILE} (guest mode — no Clerk keys)"
  fi
fi
