...
if [[ -f "${ENV_FILE}" ]]; then
  ...
  # Auto-detect the server's public IP as a sensible default.
  DETECTED_IP=$(curl -sf https://api.ipify.org 2>/dev/null || hostname -I | awk '{print $1}')
  DEFAULT_URL="http://${DETECTED_IP}:8085"
  echo ""
  echo -e "  ${BOLD}What is the public URL of this server?${RESET}"
  echo -e "  Used for Google OAuth callbacks and email links."
  echo -e "  Press Enter to accept the default."
  echo ""
  read -rp "  BETTER_AUTH_URL [${DEFAULT_URL}]: " INPUT_URL
  BETTER_AUTH_URL="${INPUT_URL:-${DEFAULT_URL}}"

  # ── Google OAuth (optional) ───────────────────────────────────────────────────
  echo ""
  echo -e "  ${BOLD}Google OAuth (optional — skip with Enter to use email/password only)${RESET}"
  echo -e "  Get credentials at: console.cloud.google.com → APIs & Services → Credentials"
  echo -e "  Redirect URI to register: ${BETTER_AUTH_URL}/api/auth/callback/google"
  echo ""
  read -rp "  GOOGLE_CLIENT_ID     (leave blank to skip): " GOOGLE_CLIENT_ID
  if [[ -n "${GOOGLE_CLIENT_ID}" ]]; then
    read -rp "  GOOGLE_CLIENT_SECRET (leave blank to skip): " GOOGLE_CLIENT_SECRET
  fi
  ...
  if [[ -n "${GOOGLE_CLIENT_ID:-}" ]]; then
    ...
    success "Secrets written to ${ENV_FILE} (email/password login enabled)"
  fi
fi
