...
Name: "{group}\{#MyAppName}"; Filename: "{sys}\cmd.exe"; \
  ...
  Comment: "Open Linux Labs in your browser"
...
begin
  ...
  SaveStringToFile(EnvFile,
    '# Auto-generated by Linux Labs installer — do not edit manually' + #13#10 +
  ...
  begin
    MsgBox('Failed to install the Windows service. The app files are in place but ' +
           'Linux Labs will not start automatically on boot.' + #13#10 +
           'You can start it manually with: docker compose up from ' + AppDir,
           mbError, MB_OK);
    Exit;
  end;

  WizardForm.StatusLabel.Caption := 'Starting Linux Labs service...';
  ExecOK(AppDir + '\{#MyAppExeName}', 'start', AppDir);

  // ── Step 8: Done ─────────────────────────────────────────────────────────
  WizardForm.StatusLabel.Caption := 'Installation complete.';
  MsgBox(
    'Linux Labs is installed and running!' + #13#10#13#10 +
